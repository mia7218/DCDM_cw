...
# Load data
mouse_data <- read.csv("clean_data-2.csv", stringsAsFactors = FALSE)
...
server <- function(input, output, session) {
  ...
  output$phenotype_bubble <- renderPlotly({
    ...
    p <- ggplot(data, aes(x = reorder(gene_symbol, pvalue),
                        ...
                        color = Significant,
                        text = tooltip)) +
      geom_point(size = 2) + ggtitle("Gene Significance for Selected Phenotype") + labs(x = "Gene", y = "-log10(p-value)") +
      scale_color_manual(values = c("Not Significant" = "grey70", "Significant" = "#ff6b6b")) +
      theme_bw() + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
